...
def counter(text):
    ...
    count_e = address.count('f')
    alphabet = 0
    for achar in text:
        if achar in lowerAlpha or achar in upperAlpha:
            alphabet += 1
    percentages = round((count_e / alphabet) * 100, 4)
    print("The Gettysburg address contains", alphabet, "characters,", "of which,",  count_e,
          "(", percentages, "% ) are",  "'f'")
